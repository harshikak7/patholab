...
const TestCard = ({ item }) => {
  ...
  return (
    <div className="bg-white rounded-[32px] overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition">
      {/* TOP */}

      <div className="p-7">
        {/* TITLE */}

        <h2 className="text-[34px] leading-[1.1] font-bold text-[#1677FF]">
          {item.testName}
        </h2>
      ...
      <div className="border-t border-gray-200 px-7 py-6 flex items-center justify-between">
        <button
          onClick={() => navigate(`/booking/${item._id}`)}
          className="bg-[#2F80FF] hover:bg-[#166CFF] text-white text-[17px] font-medium px-8 py-4 rounded-full transition"
        >
          Book Now
        </button>
